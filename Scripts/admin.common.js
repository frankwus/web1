...
function Export(fileName, htmlContent) {

    var formBody =
    "<input data-val='true' id='FileName' name='FileName' type='hidden' value='" + fileName + "'>" +
    "<input data-val='true' id='HtmlContent' name='HtmlContent' type='hidden' value='" + htmlContent + "'>";
    var filedownload = "<form action='/admin/Common/ExportToExcel' method='post'>" + formBody + "</form>";
    jQuery(filedownload).appendTo('body').submit().remove();
}